...
MonteRAMState = namedtuple("MonteRAMState", ["player_x", "player_y", "has_key", "door_left_locked", "door_right_locked", "skull_x"])
...
class MontezumaInfoWrapper(gym.Wrapper):
    ...
    def get_ram(self):
        return unwrap(self.env).ale.getRAM()
    ...
    def parse_ram(self, ram):
        ...
        level = get_byte(ram, 0xb9)
        screen = get_byte(ram, 0x83)
        skull_offset = [22, 23, 12][level] if screen == 18 else 33
        skull_x = get_byte(ram, 0xaf) + skull_offset

        return MonteRAMState(x, y, has_key, door_left_locked, door_right_locked, skull_x)
    ...
    def step(self, action):
        ...
        return obs, rew, done, info, self.get_ram()
...
class AtariEnvironment(Environment):
    ...
    def run(self):
        ...
        while True:
            ...
            s, reward, done, info, ram = self.env.step(action)
            ...
            self.child_conn.send(
                [self.history[:, :, :], reward, force_done, done, log_reward, ram])
