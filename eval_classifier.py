...
if __name__=='__main__':
    ...
    train_raw_ram_trajs, train_ram_trajs, train_frame_trajs = parse_trajectories(trajs_generator, start=0, end=199)
    test_raw_ram_trajs, test_ram_trajs, test_frame_trajs = parse_trajectories(trajs_generator, start=0, end=99)

    second_room_frame_train_states = test_frame_trajs[52] + test_frame_trajs[72]
    second_room_frame_test_states = test_frame_trajs[78]

    #test_raw_ram_trajs = test_raw_ram_trajs[0:52] + test_raw_ram_trajs[53:72] + test_raw_ram_trajs[73:102]
    #test_ram_trajs = test_ram_trajs[0:52] + test_ram_trajs[53:72] + test_ram_trajs[73:102]
    #test_frame_trajs = test_frame_trajs[0:52] + test_frame_trajs[53:72] + test_frame_trajs[73:102]
    ...
    if args.label_extractor == 'OracleExtractor':
        ...
        window_sz_hyperparms = range(1, 4)
        #window_sz_hyperparms = range(3, 4)
    ...
    for num_clusters in range(50, 100, 10):
        for num_sift_keypoints in range(40, 70, 5):
            ...
            if args.feature_extractor == 'BOVW':
                ...
                import random
                #traj_idx_24, state_idx_24 = random.choice(subgoals_info[(24, 235, 1)]['ground_truth_idxs'])
                #traj_idx_130, state_idx_130 = random.choice(subgoals_info[(130, 235, 1)]['ground_truth_idxs'])
                #traj_idx_24, state_idx_24 = 0, 0
                #traj_idx_130, state_idx_130 = 0, 0

                #train_states = [state for traj in train_frame_trajs[train_start:train_end] for state in traj] + second_room_frame_train_states + train_frame_trajs[traj_idx_24] + train_frame_trajs[traj_idx_130]
                train_states = [state for traj in train_frame_trajs[train_start:train_end] for state in traj]
                ...
                feature_extractor.visualize_sift_feats(test_frame_trajs[0][0], f"{args.dest}/sifts/")
                
                #feature_extractor.visualize_sift_feats([test_frame_trajs[traj_idx_24][state_idx_24], test_frame_trajs[traj_idx_130][state_idx_130]], f"{args.dest}/sifts/")
                #feature_extractor.visualize_sift_feats([train_frame_trajs[0][0]], f"{args.dest}/sifts/")
            ...
            for window_sz in window_sz_hyperparms:
                for nu in nu_hyperparams:
                    for gamma in gamma_hyperparams:
                        ...
                        for subgoal in subgoals:
                            print(f"[+] Running with sift={num_sift_keypoints}, clusters={num_clusters}, window_sz={window_sz}, nu={nu}, gamma={gamma} for subgoal={subgoal}")
                            ...
                            train_data, labels = label_extractor.extract_labels(train_trajs, train_raw_ram_trajs, traj_idx, state_idx)
                            ...
                            #term_classifier.feature_extractor.visualize_sift_feats(test_trajs[0][0], f"{args.dest}/sifts/")
                            ...
                            if args.term_classifier == 'OneClassSVM':
                                ...
                                file_path = f"{args.dest}/plots/sift={num_sift_keypoints}_clusters={num_clusters}_x={subgoal[0]}_y={subgoal[1]}_windowsz={window_sz}_nu={nu}_gamma={gamma}.png"
                            elif args.term_classifier == 'TwoClassSVM':
                                #file_path = f"{args.dest}/plots/x={subgoal[0]}_y={subgoal[1]}_windowsz={window_sz}_gamma={gamma}.png"
                                file_path = f"{args.dest}/plots/sift={num_sift_keypoints}_clusters={num_clusters}_x={subgoal[0]}_y={subgoal[1]}_windowsz={window_sz}_gamma={gamma}.png"
                            ...
                            save_results(args, window_sz, nu, gamma, 
                                         ...
                                         f"{args.dest}/sift={num_sift_keypoints}_clusters={num_clusters}_{subgoal[0]}_{subgoal[1]}_results.csv")
                            '''
                            save_results_sift(args, num_sift_keypoints, num_clusters, window_sz, nu, gamma, 
                                         true_pos, false_pos, ground_truth_num,
                                         precision, recall, f1,
                                         f"{args.dest}/{subgoal[0]}_{subgoal[1]}_results.csv")
