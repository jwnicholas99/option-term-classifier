...
def main():
    ...
    trajs_buffer = [[] for _ in range(num_worker)]
    completed_trajs = []

    while True:
        total_state, total_reward, total_done, total_next_state, total_action, total_int_reward, total_next_obs, total_next_ram, total_ext_values, total_int_values, total_policy, total_policy_np = \
            [], [], [], [], [], [], [], [], [], [], [], []
        ...
        for _ in range(num_step):
            ...
            next_states, rewards, dones, real_dones, log_rewards, next_obs, next_ram = [], [], [], [], [], [], []
            for i in range(len(parent_conns)):
                parent_conn = parent_conns[i]
                s, r, d, rd, lr, ram = parent_conn.recv()
                next_states.append(s)
                rewards.append(r)
                dones.append(d)
                real_dones.append(rd)
                log_rewards.append(lr)
                obs = s[-1, :, :].reshape([1, 84, 84])
                next_obs.append(obs)
                next_ram.append(ram)

                trajs_buffer[i].append((ram, obs))
            ...
            total_next_ram.append(next_ram)
            ...
            for i in range(len(real_dones)):
                if real_dones[i]:
                    completed_trajs.append(trajs_buffer[i])
                    trajs_buffer[i] = []
        ...
        if "Montezuma" in env_id:
            ...
            print(f"[+] Saving transitions to '{path}'... ", end="")
